...
void Ui::user_mode(AdoptionList* adoption_list) {
    ...
    while (!done) {
        try {
            ...
            {
                adoption_list->write_to_file(this->service);
                adoption_list->show_adoption_list();
            }*/
            else
                cout << "Bad input!" << endl;
        }
...
void Ui::start() {
    ...
    string csv_file("adoption_list.csv");
    string html_file("adoption_list.html");
    CSVAdoptionList csv_adoption_list(this->service, csv_file);
    HTMLAdoptionList html_adoption_list(this->service, html_file);
    int choice_for_file = -1;
    while (choice_for_file != 1 && choice_for_file != 2)
    {
            print_file_choice_menu();
            cin >> choice_for_file;
            cin.clear();
            cin.ignore(256, '\n');
            if (choice_for_file != 1 && choice_for_file != 2)
            {
                cout << "Invalid input!" << endl;
            }
    }
    if (choice_for_file == 1)
		adoption_list = &csv_adoption_list;
	else
		adoption_list = &html_adoption_list;
