...
DynamicArray<Dog> Service::get_all_data_service()
{
    return this->repo.get_all_dogs_repo();
}

int Service::get_capacity_service() {
    return this->repo.get_capacity();
}
...
bool Service::add_dog_service(string breed, string name, int age, string photo) {
    ...
    for (index = 0; index < length; index++) {
        string other_name = this->get_all_data_service().get_element(index).get_name();
        if (other_name == name)
            return false;
    }
...
bool Service::update_dog_service(string old_name, string new_breed, string new_name, int new_age, string new_photo) {
    ...
    this->repo.update_dog_repo(update_index, new_dog);
    return true;
}


DynamicArray<Dog> Service::get_adoption_list_service() {
...
DynamicArray<Dog> Service::get_filtered(string filter_string, int age_filter) {
    int index;
    int counter = 0;
    int length = this->repo.get_size_dogs();
    DynamicArray<Dog> valid_dogs;
    if (filter_string[0] == '\0')
    {
        for (index = 0; index < length; index++)
        {
            Dog dog = this->repo.get_dog(index);
            if (age_filter > dog.get_age())
            {
                valid_dogs.add_element(dog);
            }
        }
    }
    else {
